...
LaserscanMerger::LaserscanMerger() : Node("laserscan_multi_merger")
{
  ...
  point_cloud_publisher_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(
    cloud_destination_topic.c_str(), rclcpp::SensorDataQoS());
  laser_scan_publisher_ = this->create_publisher<sensor_msgs::msg::LaserScan>(
    scan_destination_topic.c_str(), rclcpp::SensorDataQoS());
}

rcl_interfaces::msg::SetParametersResult LaserscanMerger::reconfigureCallback(
  const std::vector<rclcpp::Parameter> & parameters)
{
...
void LaserscanMerger::laserscan_topic_parser()
{
  ...
  while (!tokens.empty()) {
    ...
    for (const auto & topic_it : topics) {
      ...
      if (
        std::find(topic_types.begin(), topic_types.end(), "sensor_msgs/msg/LaserScan") !=
          topic_types.end() &&
        tokens.erase(topic_it.first) > 0) {
  ...
  if (
    (tmp_input_topics.size() != input_topics.size()) ||
...
void LaserscanMerger::scanCallback(sensor_msgs::msg::LaserScan::SharedPtr scan, std::string topic)
{
  ...
  try {
    ...
    tf_buffer_->lookupTransform(
      scan->header.frame_id.c_str(), destination_frame.c_str(), scan->header.stamp,
      rclcpp::Duration(1, 0));
    projector_.transformLaserScanToPointCloud(
      scan->header.frame_id, *scan, tmpCloud1, *tf_buffer_, range_max);
  ...
  for (std::vector<int>::size_type i = 0; i < input_topics.size(); i++) {
    if (topic.compare(input_topics[i]) == 0) {
      ...
      if (delete_intensity) {
        pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_without_intensity(
  ...
  if (totalClouds == clouds_modified.size()) {
    ...
    // Publish point cloud after publishing laser scan as for some reason
    // moveFromPCL is causing getPointCloudAsEigen to throw a segmentation fault
    // crash
